...
REM N.O.V.A Aether OS - Low Performance Mode Startup (Windows)
...
REM Create low performance mode marker file
...
start "AetherOS Server (Low Perf)" python server.py --port %PORT% --low-perf

REM Wait for server to start
timeout /t 2 /nobreak >nul

REM Open browser with low perf parameter
start http://localhost:%PORT%/?low_perf=1

echo   [OK] Server is running. Press any key to stop...
pause >nul

REM Stop server and cleanup
taskkill /fi "WindowTitle eq AetherOS Server*" /f >nul 2>&1
if exist "static\perf-mode.json" del "static\perf-mode.json"
